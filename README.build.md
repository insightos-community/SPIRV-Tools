# SPIRV-Tools: reproducible platform builds
...
Validated musl tag: [`musl-v2025.3-4`](https://github.com/insightos-community/SPIRV-Tools/releases/tag/musl-v2025.3-4);
source commit: `0b2241da2e8b1b54bd729bfbcd1e17271c848a64`. Use a normal clone so container packaging can read `.git`.

```bash
git clone https://github.com/insightos-community/SPIRV-Tools.git SPIRV-Tools-repro
cd SPIRV-Tools-repro
git checkout --detach musl-v2025.3-4
test "$(git rev-parse HEAD)" = 0b2241da2e8b1b54bd729bfbcd1e17271c848a64
```

Native build prerequisites: C++ compiler, CMake, Ninja, Python and Git. `utils/git-sync-deps` obtains SPIRV-Headers and GoogleTest at revisions specified by DEPS.
...
cmake -S . -B build-glibc -G Ninja \
  ...
  -DSPIRV_SKIP_TESTS=OFF -DSPIRV_TOOLS_BUILD_STATIC=OFF -DSPIRV_WERROR=OFF
...
REPRO_WORK="$(mktemp -d "${TMPDIR:-/tmp}/SPIRV-Tools-musl.XXXXXXXX")"
...
- `spirv-tools-2025.3-musl-x86_64-prefix.tar.gz`
...
```bash
gh workflow run musl-release.yml --repo insightos-community/SPIRV-Tools --ref musl-v2025.3-4
gh run list --repo insightos-community/SPIRV-Tools --workflow musl-release.yml --limit 5
# Set REPRO_RUN_ID to the run ID printed above.
gh run watch "$REPRO_RUN_ID" --repo insightos-community/SPIRV-Tools --exit-status
gh run download "$REPRO_RUN_ID" --repo insightos-community/SPIRV-Tools --name musl-dist --dir downloaded-dist
```
